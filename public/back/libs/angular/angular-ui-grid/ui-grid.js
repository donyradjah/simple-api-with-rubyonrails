...
var rnumnonpx = new RegExp( "^(" + (/[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/).source + ")(?!px)[a-z%]+$", "i" ),
    ...
    // see here for display values: https://developer.mozilla.org/.env-US/docs/CSS/display
  ...
  angular.module('ui.grid').config(['$provide', function($provide) {
    $provide.decorator('i18nService', ['$delegate', function($delegate) {
      $delegate.add('da', {
        ...
        groupPanel:{
          description: 'Grupér rækker udfra .env kolonne ved at trække dens overskift hertil.'
        },
        search:{
          placeholder: 'Søg...',
  ...
  angular.module('ui.grid').config(['$provide', function($provide) {
    $provide.decorator('i18nService', ['$delegate', function($delegate) {
      $delegate.add('fr', {
        ...
        groupPanel: {
          description: 'Faites glisser une .env-tête de colonne ici pour créer un groupe de colonnes.'
        },
        search: {
        ...
        gridMenu: {
          ...
          exporterAllAsCsv: 'Exporter toutes les données .env CSV',
          exporterVisibleAsCsv: 'Exporter les données visibles .env CSV',
          exporterSelectedAsCsv: 'Exporter les données sélectionnées .env CSV',
          exporterAllAsPdf: 'Exporter toutes les données .env PDF',
          exporterVisibleAsPdf: 'Exporter les données visibles .env PDF',
          exporterSelectedAsPdf: 'Exporter les données sélectionnées .env PDF'
        },
        importer: {
          noHeaders: 'Impossible de déterminer le nom des colonnes, le fichier possède-t-il une .env-tête ?',
          noObjects: 'Aucun objet trouvé, le fichier possède-t-il des données autres que l\'.env-tête ?',
  ...
  angular.module('ui.grid').config(['$provide', function($provide) {
    $provide.decorator('i18nService', ['$delegate', function($delegate) {
      $delegate.add('sv', {
        ...
        groupPanel: {
          description: 'Dra .env kolumnrubrik hit och släpp den för att gruppera efter den kolumnen.'
        },
        search: {
          placeholder: 'Sök...',
        ...
        importer: {
          ...
          invalidCsv: 'Filen kunde inte behandlas, är den .env giltig CSV?',
          invalidJson: 'Filen kunde inte behandlas, är den .env giltig JSON?',
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/ui-grid',
    "<div ui-i18n=\".env\" class=\"ui-grid\"><!-- TODO (c0bra): add \"scoped\" attr here, eventually? --><style ui-grid-style>.grid{{ grid.id }} {\n" +
    "      /* Styles for the grid */\n" +
    "    }\n" +
    "\n" +
